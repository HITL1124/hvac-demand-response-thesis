...
% 2) 基于温度状态预测 + 热泵机理模型计算热泵功率，并与 Dymola 真值对比
...
%   processedf_Fandata.mat   : N×2, [SupFan_P(W), RetFan_P(W)]
...
% --- 热泵机理功率 ---
[Php_ol, COP_ol, Qcoil_ol, ~] = calcHpPowerCarnotEpsNTU(Tsup, mAir, Tout, Ti_ol, hpPar);
[Php_1s, COP_1s, Qcoil_1s, ~] = calcHpPowerCarnotEpsNTU(Tsup, mAir, Tout, Ti_1s, hpPar);
...
% =========================
% 拟合模型：P_tot(m) = p1*m^3 + p2*m^2 + p3*m + p4
% 说明：该拟合对应"送风机功率 + 回风机功率"的总和（单位：W）
fanPar = struct();
...
% 读取 Dymola 风机真值：processedf_Fandata.mat（N×2）
% 第一列：送风机功率(W)，第二列：回风机功率(W)
...
if isfile(fanTruthFile)
    ...
    if ~isempty(Fdat)
        if size(Fdat,1) ~= N
            ...
            Pfan_sup_sim = Fdat(:,1);
            Pfan_ret_sim = Fdat(:,2);
            Pfan_tot_sim = Pfan_sup_sim + Pfan_ret_sim; % Dymola 风机总功率（W）
        end
    end
else
...
if ~isempty(Qcon_sim)
    ...
    legend("盘管机理Qcoil(OL室温)", "盘管机理Qcoil(1-step室温)", "Dymola QCon_flow", "Location","best");
else
...
function [Php, COP, Qcoil, Tret] = calcHpPowerCarnotEpsNTU(Tsup, mAir, Tout, Ti, par)
...
% -------- 默认/保护 --------
if ~isfield(par,'r_nominal') || isempty(par.r_nominal)
    par.r_nominal = 2/3;
end
...
if ~isfield(par,'TwIn_nominal') || isempty(par.TwIn_nominal)
    par.TwIn_nominal = median(Tsup);
end
if ~isfield(par,'nAir') || isempty(par.nAir)
    par.nAir = 0.80;
end
if ~isfield(par,'tauCon') || isempty(par.tauCon)
    par.tauCon = 0;
end
if ~isfield(par,'Ts') || isempty(par.Ts)
    par.Ts = 60;
end
...
% function [Php, COP, Qcoil, Tret, epsHX] = calcHpPowerCarnotEpsNTU(Tsup, mAir, Tout, Ti, par)
% % calcHpPowerCarnotEpsNTU
% % 盘管：常数 UA 的逆流 ε-NTU（显热）
% % 热泵：Carnot_TCon 结构 COP（含部分负荷），并通过源侧能量平衡迭代自洽求解
% %
% % 输入:
% %   Tsup : N×1 供水温度 (K) -> 冷凝侧入口温度基准
% %   mAir : N×1 送风质量流量 (kg/s) -> 盘管空气侧流量
% %   Tout : N×1 室外温度 (K) -> 蒸发侧源侧入口温度近似
% %   Ti   : N×1 室内温度 (K) -> 用于混合风温度
% %   par  : 参数结构体，需包含至少：
% %          beta, UA, cp_air, cp_w, mw,
% %          Qcon_nom, Qcon_max, COP_nom,
% %          Tcon_nom, Teva_nom, TAppCon, TAppEva,
% %          mEva, aPL,
% %          (可选) tauCon, Ts
% %
% % 输出:
...
% 
% N = numel(Tsup);
% Tsup = Tsup(:); mAir = mAir(:); Tout = Tout(:); Ti = Ti(:);
% 
% Php   = zeros(N,1);
% COP   = zeros(N,1);
% Qcoil = zeros(N,1);
...
% 
% % ---------- 参数缺省保护 ----------
% if ~isfield(par,'beta') || isempty(par.beta), par.beta = 0; end
% if ~isfield(par,'UA') || isempty(par.UA), error("par.UA (W/K) must be provided."); end
% if ~isfield(par,'cp_air') || isempty(par.cp_air), par.cp_air = 1006; end
% if ~isfield(par,'cp_w') || isempty(par.cp_w), par.cp_w = 4200; end
% if ~isfield(par,'mw') || isempty(par.mw), error("par.mw (kg/s) must be provided."); end
% if ~isfield(par,'Qcon_nom') || isempty(par.Qcon_nom), error("par.Qcon_nom must be provided."); end
% if ~isfield(par,'Qcon_max') || isempty(par.Qcon_max), par.Qcon_max = inf; end
% if ~isfield(par,'COP_nom') || isempty(par.COP_nom), error("par.COP_nom must be provided."); end
% if ~isfield(par,'Tcon_nom') || isempty(par.Tcon_nom), error("par.Tcon_nom must be provided."); end
% if ~isfield(par,'Teva_nom') || isempty(par.Teva_nom), error("par.Teva_nom must be provided."); end
% if ~isfield(par,'TAppCon') || isempty(par.TAppCon), par.TAppCon = 5; end
% if ~isfield(par,'TAppEva') || isempty(par.TAppEva), par.TAppEva = 5; end
% if ~isfield(par,'mEva') || isempty(par.mEva), error("par.mEva must be provided."); end
% if ~isfield(par,'aPL') || isempty(par.aPL), par.aPL = [1,0,0]; end
...
% den0 = par.Tcon_nom + par.TAppCon - (par.Teva_nom - par.TAppEva);
% den0 = max(den0, 0.5);
% COP_carnot_nom = par.Tcon_nom / den0;
% etaCarnot0 = par.COP_nom / max(COP_carnot_nom, 1.01);
% 
% % ---------- 供水温度一阶滞后 ----------
% Tin_prev = Tsup(1);
% if par.tauCon > 0
...
% for k = 1:N
%     % 1) 空气侧入口（混合风）
%     Tain = par.beta*Tout(k) + (1-par.beta)*Ti(k);
% 
%     % 2) 水侧入口（可选滞后）
%     Tin_sp = Tsup(k);
%     Tin = Tin_prev + alpha*(Tin_sp - Tin_prev);
...
%     Cmax = max(Cair, Cw);
%     Cr   = Cmin / max(Cmax, eps_small);
%     NTU  = UA0 / max(Cmin, eps_small);
% 
%     if abs(Cr-1) < 1e-8
%         eps_k = NTU/(1+NTU);
%     else
%         eps_k = (1 - exp(-NTU*(1-Cr))) / (1 - Cr*exp(-NTU*(1-Cr)));
%     end
%     eps_k = min(max(eps_k,0),1);
%     epsHX(k) = eps_k;
% 
%     Qraw = eps_k*Cmin*(Tin - Ti(k));   % 供暖：Tin > Tain 时为正
%     Qk = max(0, Qraw);
%     Qk = min(Qk, par.Qcon_max);
% 
%     Qcoil(k) = Qk;
%     Tret(k)  = Tin - Qk / max(Cw, eps_small);
% 
%     % 4) 部分负荷效率
%     yPL = min(max(Qk / max(par.Qcon_nom, eps_small), 0), 1);
%     a1 = par.aPL(1); a2 = par.aPL(2); a3 = par.aPL(3);
